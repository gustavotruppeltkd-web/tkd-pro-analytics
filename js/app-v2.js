...
async function syncToSupabase() {
    ...
    try {
        ...
        // PROTEÇÃO GERAL contra perda acidental: os campos "biblioteca" construídos
        // pelo usuário (mesociclos, templates, exercícios, questionários) só entram
        // no patch quando NÃO estão vazios. updateSettings faz upsert PARCIAL —
        // omitir a chave preserva o valor do servidor. Assim um estado local vazio
        // (corrida, db "lite"/parcial, device stale) NUNCA apaga esses dados no
        // servidor. db._allowEmptySingletons=true permite exclusão deliberada
        // (setado nos deletes que podem zerar a lista).
        const _libFields = {
        ...
        // db._allowEmptyAvatar=true permite remover de propósito.
